    ...
    <div class="diary-card" onclick="openDiaryModal(${d.id})">
      ...
      <div class="diary-right">
        ...
        <div style="margin-top:8px;display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
          ...
          ${isAdmin ? `
            <button class="rec-edit-btn" onclick="event.stopPropagation(); editDiary(${d.id})">✏️ 수정</button>
            <button class="delete-btn"   onclick="event.stopPropagation(); deleteDiary(${d.id})">🗑️ 삭제</button>` : ''}
        </div>
      </div>
    </div>`).join('');
}
...
function openDiaryModal(id) {
  ...
  const adminBtns = isAdmin ? `
    <div class="admin-actions" style="margin-top:14px;">
      <button class="rec-edit-btn" onclick="closeModalBtn(); editDiary(${d.id})">✏️ 수정</button>
      <button class="delete-btn"   onclick="closeModalBtn(); deleteDiary(${d.id})">🗑️ 삭제</button>
    </div>` : '';
...
function initEmojiPicker(initial) {
...
function editDiary(id) {
  ...
  document.getElementById('write-form-content').innerHTML = `
    ...
    <div class="form-group">
      <label>기분 한 마디</label>
      <input type="text" id="d-mood" value="${d.mood}" placeholder="예: 기분 좋음">
    </div>
    <div class="form-group">
      <label>오늘의 이야기 <span style="color:var(--peach);">*</span></label>
      <textarea id="d-full" style="min-height:130px;">${d.full}</textarea>
    </div>
    <div class="form-msg" id="d-msg">내용은 필수예요!</div>
    <div class="form-actions">
      <button class="btn-cancel" onclick="closeWriteModal()">취소</button>
      <button class="btn-save"   onclick="updateDiary(${id})">저장</button>
    </div>`;
  document.getElementById('write-overlay').classList.add('open');
  initEmojiPicker(d.emoji);
  setTimeout(() => { const el = document.getElementById('d-full'); if (el) el.focus(); }, 100);
}

function updateDiary(id) {
  const idx = diaryData.findIndex(x => x.id === id);
  if (idx === -1) return;
  const full = document.getElementById('d-full').value.trim();
  if (!full) { document.getElementById('d-msg').style.display = 'block'; return; }
  const mood    = document.getElementById('d-mood').value.trim() || '기록';
  const preview = full.length > 60 ? full.slice(0, 60) + '...' : full;

  diaryData[idx] = { ...diaryData[idx], emoji: selectedEmoji, mood, preview, full };
  saveDiaryLS();
  closeWriteModal();
  renderDiary();
}
