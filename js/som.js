...
function showSomDetail(key) {
  ...
  const rows = events.map((e, idx) => {
    const adminBtns = isAdmin
      ? `<button class="rec-edit-btn" style="padding:3px 10px;margin-left:4px;"
           onclick="editSomEvent('${key}',${idx})">✏️</button>
         <button class="delete-btn" style="padding:3px 10px;margin-left:4px;"
    ...
    return `<div style="display:flex;align-items:center;padding:7px 0;border-bottom:1px solid var(--peach-pale);">
      <span style="flex:1;font-size:0.95rem;">${e}</span>${adminBtns}
    </div>`;
  }).join('');
...
function openSomRealPhoto(i) {
  ...
  const adminBtns = isAdmin
    ? `<div class="admin-actions" style="margin-top:14px;">
        <button class="rec-edit-btn" onclick="editSomPhotoCaption(${i})">✏️ 설명 수정</button>
        <button class="delete-btn"   onclick="closeModalBtn(); deleteSomPhoto(${i})">🗑️ 삭제</button>
       </div>`
    : '';
  ...
  document.getElementById('modal-content-area').innerHTML = `
    ...
    <div class="modal-content" style="font-size:0.9rem;" id="photo-caption-${i}">${g.caption}</div>
    ${adminBtns}`;
  openModal();
}
...
/* ════════════════════════════════
   솜 일정 수정
════════════════════════════════ */
function editSomEvent(key, idx) {
  if (!isAdmin) return;
  const current = (somEvents[key] && somEvents[key][idx]) || '';
  document.getElementById('write-form-content').innerHTML = `
    <div class="write-modal-title">✏️ 일정 수정</div>
    <div class="form-group">
      <label>빠른 선택</label>
      <select id="se-preset" onchange="applySomEditPreset()">
    ...
    <div class="form-group">
      <label>일정 내용 <span style="color:var(--peach);">*</span></label>
      <input type="text" id="se-event" value="${current.replace(/"/g,'&quot;')}"
             onkeydown="if(event.key==='Enter') updateSomEvent('${key}',${idx})">
    </div>
    <div class="form-msg" id="se-msg">내용을 입력해 주세요!</div>
    <div class="form-actions">
      <button class="btn-cancel" onclick="closeWriteModal(); closeModalBtn();">취소</button>
      <button class="btn-save"   onclick="updateSomEvent('${key}',${idx})">저장</button>
    </div>`;
  document.getElementById('write-overlay').classList.add('open');
  setTimeout(() => { const el = document.getElementById('se-event'); if (el) { el.focus(); el.select(); } }, 100);
}

function applySomEditPreset() {
  const v = document.getElementById('se-preset').value;
  if (v) document.getElementById('se-event').value = v;
}

function updateSomEvent(key, idx) {
  const val = document.getElementById('se-event').value.trim();
  if (!val) { document.getElementById('se-msg').style.display = 'block'; return; }
  if (!somEvents[key]) return;
  somEvents[key][idx] = val;
  saveSomEvtLS();
  closeWriteModal();
  closeModalBtn();
  renderSomCalendar();
...
/* ════════════════════════════════
   솜 사진 설명 수정
════════════════════════════════ */
function editSomPhotoCaption(i) {
  if (!isAdmin) return;
  const g = somGallery[i];
  if (!g) return;
  document.getElementById('write-form-content').innerHTML = `
    <div class="write-modal-title">✏️ 사진 설명 수정</div>
    <div class="form-group">
      <label>사진 설명</label>
      <input type="text" id="spc-input" value="${(g.caption || '').replace(/"/g,'&quot;')}"
             placeholder="예: 목욕 후 솜이 🛁"
             onkeydown="if(event.key==='Enter') updateSomCaption(${i})">
    </div>
    <div class="form-actions">
      <button class="btn-cancel" onclick="closeWriteModal()">취소</button>
      <button class="btn-save"   onclick="updateSomCaption(${i})">저장</button>
    </div>`;
  document.getElementById('write-overlay').classList.add('open');
  setTimeout(() => { const el = document.getElementById('spc-input'); if (el) { el.focus(); el.select(); } }, 100);
}

function updateSomCaption(i) {
  const val = document.getElementById('spc-input').value.trim();
  if (!somGallery[i]) return;
  somGallery[i] = { ...somGallery[i], caption: val || '솜이 사진' };
  saveSomGalLS();
  closeWriteModal();
  // 모달 내 설명 즉시 반영
  const captionEl = document.getElementById(`photo-caption-${i}`);
  if (captionEl) captionEl.textContent = somGallery[i].caption;
  renderSomGallery();
}
